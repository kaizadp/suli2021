...
## The sample data used here are from Patel et al. 2021 Soil Biology and Biochemistry, https://doi.org/10.1016/j.soilbio.2021.108165
...
# STEP 1b. set bins -------------------------------------------------------
## we will use bins from Clemente et al. 2012 for this exercise
## open the file to see what the 6 bins mean
bins_dat = 
...
## create the spectra graph using the gg_spectra() function, 
## and then customize as needed
gg_spectra(dat = spectra_processed, # input data file
           LABEL_POSITION = 4, # where on the y-axis do you want the bin labels?
           aes(x = ppm, y = intensity, 
               group = source, color = source),
           STAGGER = 1) + # set the separation between the spectra
  labs(subtitle = "binset: Clemente 2012")+
  ylim(0, 5) # adjust the y-axis limits (range) as needed

#
# STEP 5: relative abundance ---------------------------
## we will use peaks data to calculate relative abundance

## i. process peaks data ----
